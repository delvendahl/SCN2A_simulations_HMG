...
:
: modified 18 June 2021, Igor Delvendahl
...
NEURON {
    ...
    GLOBAL vShift_act, vShift_inact, maxrate, bh_scale, ah_scale
    RANGE vShift_inact_local
    RANGE gna, gbar, ina_x
...
:   gbar = 33     (millimho/cm2)
    ...
    vShift_act = 10        (mV)  : shift to the right for activation

    vShift_inact = 10      (mV)  : global shift to the right for inactivation
	...
	bh_scale = 1				: scaling factor for inactivation
	ah_scale = 1				: scaling factor for inactivation
...
:   ina = g*(v - ena)*(1e-3)
    ...
    ina_x = gna*(v - ena)*(1e-4) 	: define  gbar as pS/um2 instead of mllimho/cm2		:to monitor
...
:   maxrate = tadj*maxrate
    ...
    a1 = tadj*a1_0*exp( a1_1*(vS-vShift_act))
    a1 = a1*maxrate / (a1+maxrate)

    b1 = tadj*b1_0*exp(-b1_1*(vS-vShift_act))
    b1 = b1*maxrate / (b1+maxrate)

    a2 = tadj*a2_0*exp( a2_1*(vS-vShift_act))
    a2 = a2*maxrate / (a2+maxrate)

    b2 = tadj*b2_0*exp(-b2_1*(vS-vShift_act))
    b2 = b2*maxrate / (b2+maxrate)

    a3 = tadj*a3_0*exp( a3_1*(vS-vShift_act))
    a3 = a3*maxrate / (a3+maxrate)

    b3 = tadj*b3_0*exp(-b3_1*(vS-vShift_act))
